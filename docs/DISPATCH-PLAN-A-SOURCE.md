# 方案A 派单（含完整源码）—— Claude 直接产出完整文件

> 以下每个文件是当前真实源码的完整内容，请基于这些源码修改后输出完整新文件（非 diff）。
> 修改要求见文件内标注的 TODO-PLANA 或文末说明。禁止臆造未提供的 API。
...
# 修改要求（请基于上面源码修改并输出完整新文件）

## 目标：发布统一走「写记录」，文字可配图 + 可选定位城市；隐藏照片独立入口

## 需要修改的文件及要求

### 1. lib/models/memory.dart —— 不改（relatedMediaIds 已存在）

### 2. lib/providers/memory_provider.dart
- save() 增加可选参数 `List<String>? relatedMediaIds`，构造 Memory 时传入（缺省保持 const []，旧调用/测试不破坏）。

### 3. lib/screens/memory/memory_edit_screen.dart（重点）
- 在正文输入框与标签组件之间加「添加照片」按钮：点击弹底部选择 拍照 / 从相册选。
- 配图实现（必须复用现有照片体系）：
  1) 用 ImagePicker().pickImage(source: camera/gallery)；
  2) 压缩：img.decodeImage(bytes) → maxSide>512 时 copyResize 512 → img.encodeJpg(quality:80)；
  3) 存文件：getApplicationDocumentsDirectory()/photos/{uuid}.jpg（目录不存在先创建）；
  4) 构造 Photo(id:uuid, localPath, takenAt: DateTime.now().toUtc(), tags:[], relatedMemoryIds:[], metadata:{}, createdAt: now) 并 await PhotoProvider.addPhoto(photo)；
  5) 把 photo.id 加入本页待保存列表，UI 显示缩略图预览（可多张上限 3；每张带删除按钮 = 从列表移除该 id，不删 Photo 记录本身）。
  - 编辑已有记忆时 initState 若 memory.relatedMediaIds 非空：从 PhotoProvider.byId(id) 加载预览（跳过加载失败的 id）。
  - _onSave 时把待保存 photoId 列表传给 provider.save(relatedMediaIds: [...])。
  - 页面需要 context.read<PhotoProvider>()；需要 import 白名单内包：image_picker/image/path_provider/uuid。
  - 新建与编辑保存后返回前，若从未新增/删除关联，relatedMediaIds 与原值一致即可（避免误清空）。

### 4. lib/screens/record/records_screen.dart
- SegmentedButton 从 3 段改为 2 段：全部 / 文字（删除「照片」段）；IndexedStack children 去掉 PhotoTimelineScreen。
- AppBar PopupMenuButton 删除 value=="photo" 项与对应跳转（只保留「写记录」→MemoryEditScreen）。
- FAB：删除 `if (_segment == 2)` 分支，统一 push MemoryEditScreen。
- _MixedRecordsView 去重：照片若已被某条 Memory 的 relatedMediaIds 引用，则不再作为独立 _PhotoRecordTile 出现在流中（用被引用 photoId 集合过滤 photos）。
- _MemoryRecordTile：若 memory.relatedMediaIds 非空，取第一张照片（PhotoProvider.byId）在文字下方显示缩略图（高约 150、圆角、Image.file），点击缩略图 → Navigator.push PhotoDetailScreen(photo)。

### 5. lib/screens/home/home_screen.dart
- 删除「拍一张照片」_quickAction 块（icon: Icons.photo_camera_rounded, title: '拍一张照片' 的那个 Expanded 子项）及其对应 Navigator.push；保留「写一条」「打卡」两张卡。
- 若删除后该行只有 2 卡，布局自动等分即可（Row + 两个 Expanded 已满足）。
- 清理不再使用的 import（如 PhotoTimelineScreen 若仅此处引用）。

## 禁止
- 不新建"图片嵌入 Memory 表"或改 Photo 表结构——照片仍是独立 Photo 记录，Memory.relatedMediaIds 只存 id。
- 不删除 PhotoTimelineScreen/PhotoDetailScreen 文件、不删 PhotoProvider/PhotoRepository 能力。
- 不改 schema.sql / pubspec.yaml / 白名单。
- 不在 UI 直接处理原始 Map（R10）。

## 验收自测
- flutter analyze 0 issues；flutter test 全绿（现有 277；save 参数可选缺省 [] 不应破坏）。
- 交付：改动文件清单 + 每个改动文件的完整新版本（非 diff），文件头保留 `// lib/...` 注释。

---

# 补充：lib/screens/home/home_screen.dart 相关片段（只展示需要修改的区域）
...
```
## 快捷卡位置提示（行号仅供参考，按代码内容匹配）
```

```
## 快捷卡区域实际代码（约 L255-349，含「写一条」「拍一张照片」「打卡」三卡，需删除「拍一张照片」整块）
```dart
          Text(
            date,
            style: const TextStyle(
