...
export const SettingsModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
      <div className={`${themeColors.cardBg} border ${themeColors.cardBorder} rounded-2xl w-full max-w-lg shadow-xl p-6 relative max-h-[90vh] overflow-y-auto`}>
        <button
        ...
        <h3 className={`text-base font-semibold ${themeColors.textMain} mb-4`}>系统设置与个人偏好</h3>

        <div className="space-y-5">
          {/* Section 1: Visual Eye-care Theme (3 Options) */}
          <div className={`p-3.5 ${themeColors.subtleBg} rounded-xl border ${themeColors.subtleBorder}`}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <Palette className={`w-4 h-4 ${themeColors.primaryText}`} />
                <label className={`text-xs font-semibold ${themeColors.textMain}`}>
                  视觉护眼主题配色 (3种风格可选)
                </label>
              </div>
              <span className={`text-[10px] ${themeColors.textSub}`}>缓解视觉疲劳 · 随心切换</span>
            </div>
            <p className={`text-[11px] ${themeColors.textMuted} mb-3 leading-relaxed`}>
              ...
              <button
                ...
                className={`p-3 rounded-xl border text-left transition-all relative flex flex-col justify-between ${
                  theme === 'sky'
                    ...
                    : `${themeColors.subtleBorder} ${themeColors.cardBg} ${themeColors.subtleHoverBg}`
                }`}
              >
                <div>
          ...
          <div className={`p-3.5 ${themeColors.subtleBg} rounded-xl border ${themeColors.subtleBorder}`}>
            ...
            <p className={`text-[11px] ${themeColors.textMuted} mb-2.5 leading-relaxed`}>
            ...
            <div className="flex flex-wrap gap-1.5 mb-3">
              {taskCategories.map((cat) => (
                <span
                  key={cat}
                  className={`inline-flex items-center gap-1 px-2.5 py-1 ${themeColors.cardBg} border ${themeColors.cardBorder} ${themeColors.textMain} rounded-lg text-xs shadow-2xs group`}
                >
                  <span>{cat}</span>
                  <button
                    type="button"
                    ...
                    className={`${themeColors.textSub} hover:text-rose-500 transition-colors p-0.5`}
                    title={`删除「${cat}」分类`}
                  >
                    <X className="w-3 h-3" />
                  </button>
            ...
            <div className="flex gap-2">
              <input
                ...
                className={`flex-1 text-xs py-1.5 px-3 ${themeColors.cardBg} border ${themeColors.cardBorder} ${themeColors.textMain} rounded-lg focus:outline-none focus:border-purple-500`}
              />
              <button
                type="button"
          ...
          <div className="p-3.5 bg-emerald-50/50 rounded-xl border border-emerald-200/80">
            ...
            <div className="space-y-2 mb-3">
              <div>
                <label className={`block text-[11px] font-medium ${themeColors.textSub} mb-0.5`}>
                  Project URL:
                </label>
                <input
                  type="text"
                  ...
                  className={`w-full text-xs py-1.5 px-3 ${themeColors.cardBg} border ${themeColors.cardBorder} ${themeColors.textMain} rounded-lg focus:outline-none focus:border-emerald-500 font-mono text-[11px]`}
                />
              </div>
              <div>
                <label className={`block text-[11px] font-medium ${themeColors.textSub} mb-0.5`}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full text-xs py-1.5 pl-3 pr-8 ${themeColors.cardBg} border ${themeColors.cardBorder} ${themeColors.textMain} rounded-lg focus:outline-none focus:border-emerald-500 font-mono text-[11px]`}
                  />
                  <button
                    type="button"
                    onClick={() => setShowAnonKey(!showAnonKey)}
                    className={`absolute right-2 top-1/2 -translate-y-1/2 ${themeColors.textSub} hover:${themeColors.textMain} p-0.5`}
              ...
              <div className="pt-1">
                <label className={`inline-flex items-center gap-2 cursor-pointer text-[11px] ${themeColors.textSub} select-none`}>
                  <input
                    type="checkbox"
            ...
            <div className="flex flex-wrap items-center gap-2 mb-2">
              ...
              <button
                ...
                className={`px-2.5 py-1.5 ${themeColors.cardBg} hover:bg-emerald-100/60 border border-emerald-300 text-emerald-800 disabled:opacity-40 text-xs font-medium rounded-lg flex items-center gap-1 transition-colors`}
              >
            ...
            {syncStatus.lastSyncedAt && (
              <div className={`text-[10px] ${themeColors.textSub} mt-2`}>
          ...
          <div className={`p-3.5 ${themeColors.subtleBg} rounded-xl border ${themeColors.subtleBorder}`}>
            <div className="flex items-center gap-2 mb-1.5">
              <Smartphone className={`w-4 h-4 ${themeColors.primaryText}`} />
              <label className={`text-xs font-semibold ${themeColors.textMain}`}>
                安卓原生 APK / PWA 移动端运行
              </label>
            </div>
            <p className={`text-[11px] ${themeColors.textSub} mb-2`}>
              本系统已配置 <strong>Capacitor Android</strong> 原生工程与 <strong>PWA 离线支持</strong>：
            </p>
            <div className={`${themeColors.cardBg} border ${themeColors.cardBorder} rounded-lg p-2.5 text-[11px] ${themeColors.textMain} space-y-1`}>
              <div>
                <strong>方法 1（立即在手机使用）：</strong>手机浏览器打开本链接，点击菜单中「<strong>添加到主屏幕</strong>」即可秒变原生 App 全屏运行，支持离线记录。
              </div>
              <div>
                <strong>方法 2（编译 APK）：</strong>项目根目录下执行 <code className={`${themeColors.subtleBg} px-1 py-0.5 rounded text-sky-700`}>npx cap sync android</code> 与 <code className={`${themeColors.subtleBg} px-1 py-0.5 rounded text-sky-700`}>npx cap open android</code>，在 Android Studio 中点击 <em>Build APK</em> 即可输出 release/debug APK。
              </div>
            </div>
          </div>

          {/* API Key */}
          <div className={`p-3 ${themeColors.subtleBg} rounded-xl border ${themeColors.subtleBorder}`}>
            <div className="flex items-center gap-2 mb-1.5">
              <Key className={`w-4 h-4 ${themeColors.primaryText}`} />
              <label className={`text-xs font-semibold ${themeColors.textMain}`}>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  placeholder="sk-..."
                  className={`w-full text-xs py-1.5 pl-3 pr-8 ${themeColors.cardBg} border ${themeColors.cardBorder} ${themeColors.textMain} rounded-lg focus:outline-none ${themeColors.focusRing}`}
                />
                <button
                  type="button"
              ...
              <button
                ...
                className={`px-3 py-1.5 ${themeColors.actionBtn} text-white text-xs font-medium rounded-lg flex items-center gap-1 shrink-0`}
              >
          ...
          {/* Backup & Reset */}
          <div className={`space-y-2 pt-2 border-t ${themeColors.divider}`}>
            <div className="flex items-center justify-between">
              <span className={`text-xs ${themeColors.textMain} font-medium`}>数据导出备份</span>
              <button
                type="button"
                onClick={handleExportData}
                className={`flex items-center gap-1 px-3 py-1.5 ${themeColors.subtleBg} ${themeColors.subtleHoverBg} ${themeColors.textMain} border ${themeColors.subtleBorder} text-xs rounded-lg transition-colors`}
              >
                <Download className="w-3.5 h-3.5" />
                <span>导出 JSON</span>
              </button>
            ...
            <div className="flex items-center justify-between pt-2">
              <div>
                ...
                <span className={`text-[10px] ${themeColors.textSub}`}>重置并加载标准的完整测试样例</span>
              </div>
              <button
                type="button"
                onClick={handleResetData}
                className="flex items-center gap-1 px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs rounded-lg transition-colors border border-rose-200"
              >
                <RefreshCw className="w-3.5 h-3.5" />
                <span>重置数据</span>
              </button>
